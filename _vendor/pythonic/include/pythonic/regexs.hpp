...
namespace pythonic {
    namespace regexs {
        ...
        string sub(regex const &pattern_re, UnaryFunction f, string_view haystack, size_t n=-1) {
          string s;
        ...
        template <class UnaryFunction>
        auto sub(string_view pattern, UnaryFunction f, string_view haystack, size_t n=-1, regex::flag_type regex_flag = regex::ECMAScript) {
          return sub(regex(pattern.data(), pattern.size(), regex_flag), f, haystack, n);
        }
        //! [sub]
    }
}
